...
> **ĐÍNH CHÍNH cho hệ thống này.** Mục [Input images and references](#input-images-and-references)
> bên dưới dạy gán nhãn ảnh theo chỉ số (`Image 1: edit target`, `Image 2: style reference`).
> Với `gpt-image-2` việc đó KHÔNG có tác dụng: đảo nhãn `REFERENCE IMAGE 1/2` mà giữ
> nguyên thứ tự ảnh cho ra kết quả không đổi. Đo 2026-08-10, bằng chứng
> `work/experiments/multi_reference/20260810-074411Z_result.json` (repo Python), ghi ở
> `contracts/providers/gpt-image.json` → `capabilities.numeric_reference_labels_honoured`.
>
> **THỨ TỰ ảnh mới là thứ có tác dụng** — `media_runtime/providers/gpt_image.py` gửi
> `image[]` theo đúng thứ tự. Phần còn lại của file giữ nguyên văn.
