...
profile            design guidance, prohibited features, ordered stages,
                   required stages, scene-count limits, subject and
                   location limits, the people expected at each stage,
                   and the coverage this film must answer.
...
Work at scene level. Do not write shot lists, provider settings or
image/video generation prompts in this stage.
...
Establish these decisions before choosing the premise or writing scenes:
...
1. Read the inspiration and identify qualities worth developing.
   Separate useful ideas from source facts that must not be reproduced.

2. Establish the design thesis and the overall vessel.
   Resolve its defining form and spatial relationships before choosing
   a storytelling approach.

3. Describe the protagonist's stable visual identity.
   Identify supporting people only where their work or experience
   contributes meaningfully to the film.

4. Choose the premise around this particular design.
   The story must serve the vessel, not reshape it to fit a visual trick.

5. Develop the journey through the profile's stages in the required order.
   Choose locations and scenes that make the design understandable.

6. Write observable action, sound and optional dialogue for each scene.
   Explain what the scene contributes and how it prepares what follows.

7. Estimate scene durations from the actions being shown.

8. Review the whole screenplay for design coherence, spatial consistency,
   construction continuity, repetition and a meaningful ending.
...
Do not let the opening promise a film about one thing while the remaining
scenes deliver a film about something else.
...
Before returning the screenplay, check:

1. Can the viewer identify what distinguishes this yacht without being
   told that it is unique?

2. Does its novelty belong to the overall design rather than one amenity?

3. Do form, spatial organization and intended experience support each other?

4. Are actions physically credible without unsupported engineering claims?

5. Do spaces, materials and build states remain consistent?

6. Do people have meaningful, believable behaviour?

7. Does operation demonstrate what design promised?

8. Does the ending answer the opening question?

9. Have prohibited source details been excluded?

10. Is any scene present only because it sounds impressive?

11. Does every coverage item appear exactly once, does each required one
    point at a scene that shows it, and does each evidence line say only
    what those scenes support?

12. Does every scene carry build_state, does each one describe the object
    at that scene, and is null used only where build state does not apply
    or is not shown?

13. Do the participants in each stage match the work that stage contains,
    and does every line of dialogue belong to a person present in its scene?

A structurally valid screenplay can still be incomplete or unconvincing.
